...
if __name__ == '__main__':
    ...
    def plot_cells(ax):
        for cl, col in zip(range(3), [blue, orange, green]):
            X = X1[y1==cl,:]
            ax.scatter(X[:,0], X[:,1], color=col, label=r'cluster {} ($t_1$)'.format(cl+1))

        ax.scatter(X2[:,0], X2[:,1], color='gray', alpha=0.8, label=r'$t_2$')
        ax.set_xlabel(r'$x_1$')
        ax.set_ylabel(r'$x_2$')
        ax.legend(loc=0)
